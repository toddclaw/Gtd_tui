...
### Added
- **BACKLOG-78 — Split view (task list + detail pane)**: `\` toggles a right-side `TaskSplitPane` alongside the task list. The pane shows the selected task's title, metadata (schedule/deadline), notes (Markdown read view or VimInput edit), and checklist summary. Moving `j`/`k` updates the pane immediately. `l` focuses the pane; `h` returns focus to the task list and saves notes. Notes are also auto-saved on `Esc` (switching to COMMAND mode). Split ratio is configurable via `split_ratio` in `[ui]` of `config.toml` (default `0.6`).
- **BACKLOG-82 — Markdown-rendered notes in detail view**: Task notes in `TaskDetailScreen` now show as rendered Markdown via a `MarkdownNotesProxy` widget in COMMAND mode. Pressing `i`/`a`/`A`/`o`/`O` on the proxy switches to the raw `VimInput` in INSERT mode; pressing `Esc` from INSERT returns to the rendered Markdown view. Tasks with empty notes skip the proxy and show the VimInput directly. A new `VimInput.ModeChanged` message is posted on every mode transition.
- **BACKLOG-45 — "Anytime" folder**: Built-in "Anytime" smart view (active, unscheduled tasks) added between Today and Upcoming in the sidebar. `o`/`O` in Anytime view create tasks with `folder_id="anytime"`. `m` (move) includes Anytime as a destination. Sidebar digit shortcuts shift: `2`=Anytime, `3`=Upcoming, `4`=Waiting On, `5`=Someday.
- **BACKLOG-54 — Snooze / Defer task**: `z` opens a snooze-duration picker (1 hour, 3 hours, Tomorrow 9 am, 1 week). Snoozed tasks are hidden from Today, Upcoming, Anytime, and search until the timer expires. Expired snoozes are resolved automatically on app launch. Snoozed tasks show a `[Snoozed until Day]` indicator in the task list. `snoozed_until` field round-trips through JSON storage.
