...
- **VimInput key conflicts — use `priority=True`:** `VimInput._on_key()` calls `event.stop()` on many keys it handles (e.g. `ctrl+e` = end-of-line, `h`/`j`/`k`/`l`/`i`/`a`/`o` in COMMAND mode, `Esc`). Before adding a new binding in a screen or widget that contains a focused `VimInput`, grep `vim_input.py` for the key to confirm whether VimInput consumes it. If it does, add `priority=True` to the `Binding(...)` declaration so the screen/widget action fires first. The same applies to `TaskSplitPane`: its `h` binding uses `priority=True` to intercept `h` before VimInput handles it in COMMAND mode.
- **`App.suspend()` is synchronous:** Textual's `App.suspend()` is a plain `@contextmanager`, not an async one. Use `with self.app.suspend():` (not `async with`). In tests, mock it with `contextlib.contextmanager` (not `asynccontextmanager`). Mixing these causes a `TypeError` at runtime.
- **`GtdApp.action_cancel_insert_mode` intercepts `Escape` globally** via a priority binding. Before adding Esc handling inside a modal widget or pane (e.g. `TaskSplitPane`), check whether `action_cancel_insert_mode` will steal the event first. If so, add a focus check (`self.app.focused` is inside the pane) at the top of `action_cancel_insert_mode` to short-circuit, or use a priority binding on the inner widget itself.
